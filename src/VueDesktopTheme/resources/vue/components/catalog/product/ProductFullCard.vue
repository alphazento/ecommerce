<template>
  <v-container fluid>
        ...
        <v-content>
          ...
          <v-form :action="`/shoppingcart/add_product/${product.id}`" method="POST">
            <v-layout row>
              ...
              <v-flex md5 xs5>
                <v-btn depressed large class="btn__addtocart" type="submit">Add to Cart</v-btn>
              </v-flex>
            </v-layout>
          </v-form>
        </v-content>
    ...
    <v-layout row>
      <v-tabs v-model="model" background-color=" accent-4" center-active centered dark>
        <v-tab v-for="(tab, idx) in tabs" :key="idx" :href="`#tab-${idx}`">{{tab}}</v-tab>
      </v-tabs>
      <v-tabs-items v-model="model">
        <v-tab-item v-for="(tab, idx) in tabs" :key="idx" :value="`tab-${idx}`">
