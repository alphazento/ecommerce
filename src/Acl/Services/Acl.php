...
class Acl implements \Zento\Passport\Services\APCInterface {
    protected $user;
    ...
    public function checkRequest(\Illuminate\Http\Request $request) {
        ...
        if ($this->inGroupPermissions($request, $user)) {
            return true;
        }
        return false;
    }
    ...
    public function checkRoute($uri, $method = 'get') {
        return $this->checkRequest(Request::create($uri, strtoupper($method)));
    }
    ...
    protected function inGroupPermissions(\Illuminate\Http\Request $request, AclUserInterface $user) {
