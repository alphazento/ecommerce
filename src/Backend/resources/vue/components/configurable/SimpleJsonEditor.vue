<template>
    <component v-if="components['metanode']" :is="components['ui']" v-bind="components"
        @valueChanged="metaValueChanged"
    >
    </component>
    <v-container v-else-if="isObject(components)">
        <v-layout v-for="(element, name) of components" :key="name" :class="`group-level-${level}`">
            <v-flex md3>
                <v-btn icon v-if="Array.isArray(element)" @click="addArrayItem(name)">
                    <v-icon color="error">mdi-plus-circle</v-icon>
                </v-btn>
                <span>{{ name }}:</span>
            </v-flex>
            <v-flex md9>
                <simple-json-item :components="element" :level="1+level" 
                    :belongs="name" 
                    @removeArrayItem="removeArrayItem"
                    @jsonItemChanged="jsonItemChanged"> </simple-json-item>
            </v-flex>
        </v-layout>
    </v-container>
    <v-container v-else>
        <v-layout v-for="(element, i) of components" :key="i" class="group-bound">
