    ...
    convertToComponentArray(json, schema) {
      ...
      for (const [key, value] of Object.entries(json)) {
        if (this.isObject(value)) {
          components[key] = this.convertToComponentArray(value, schema[key]);
        } else {
          if (Array.isArray(value)) {
            ...
            for (var i = 0; i < value.length; i++) {
              var element = value[i];
              components[key].push(
                this.convertToComponentArray(element, schema[key][0])
              );
            }
          } else {
            console.log("kv", key, value, json, schema);
            components[key] = {
              text: key,
              ui: schema[key],
              value: schema[key] === value ? "" : value,
              accessor: key,
              metanode: true
            };
          }
