            ...
            <v-expansion-panels value="0" accordion mandatory>
                <v-expansion-panel>
                    <v-expansion-panel-header text-left
                        >Dynamic Attribute Set</v-expansion-panel-header
                    >
                    <v-expansion-panel-content>
                        <v-list-item>
                            <a @click.stop="navToGroup('Category')"
                                >For Category</a
                            >
                        </v-list-item>
                        <v-list-item>
                            <a @click.stop="navToGroup('Product')"
                                >For Product</a
                            >
                        </v-list-item>
                        <v-list-item>
                            <a @click.stop="navToGroup('Cutomer')"
                                >For Customer</a
                            >
                        </v-list-item>
                    </v-expansion-panel-content>
        ...
        <v-flex md10>
            <v-tabs v-model="tab" v-if="!!group">
                ...
                <v-tab v-if="editorTab">
                    <v-icon left>mdi-lock</v-icon>
                    <span class="error" v-if="editMode == 'new'"
                        >New Attribute Set</span
                    >
                        ...
                        >
                            <template v-slot:body="{ headers, items }">
                                <tbody>
                                    ...
                                    >
                                        <td
                                            v-for="(col, ci) of headers"
                                            :key="ci"
                                        >
                                            <v-btn
                                                v-if="ci == 0"
                                                icon
                                                @click="editAttributeSet(row)"
                                                color="primary"
                                            >
                                                <v-icon
...
export default {
    ...
    methods: {
        ...
        fetchDefines() {
            ...
            axios
                .get(
                    "/api/v1/admin/configs/groups/tables/dynamic-attribute-sets"
                )
                .then(response => {
